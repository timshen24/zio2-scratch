...
object GenerationPlayground extends ZIOAppDefault {
  def run = {
    val generalGenerator = Gen.unfoldGen(0)(i => Gen.const(i + 1).zip(Gen.stringN(i)(Gen.alphaNumericChar)))
    val generatedListsZIO = generalGenerator.runCollectN(100) // force generation collect 100 elements
    val generatedListsZIO_v2 = generatedListsZIO.provideLayer(Sized.live(50)) // describe the size of the sample the
    // generator will use, 50 elements
