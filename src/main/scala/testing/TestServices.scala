...
/*
  built-in test services you can directly use
  - console
  - random
  - clock
  - system
  ...
  def spec = suite("Checking built-in test services")(
    test("ZIO console application") {
      val logicUnderTest: Task[Vector[String]] = for {
        _ <- TestConsole.feedLines("Daniel") // feed to name <- Console.readLine("")
        _ <- DummyConsoleApplication.welcomeUser()
        output <- TestConsole.output // output result of welcomeUser
