...
object Refs extends ZIOAppDefault {
  ...
  def tickingClockPure(): UIO[Unit] = {
    ...
    for {
      t <- ticks
      _ <- tickingClock(t) zipPar printTicks(t)
    } yield ()
  }

  /**
   * 2 Why does this print Zero forever?
   */
  def tickingClockPure_v2(): UIO[Unit] = {
    val ticksRef: UIO[Ref[Long]] = Ref.make(0L) // Because this is just a description that will produce a Ref[Long] later

    // print the current time every 1s + increase a counter ("ticks")
    def tickingClock: UIO[Unit] = for {
      ticks <- ticksRef // evaluates and produces a new Ref[Long] every time
      _ <- ZIO.sleep(1.second)
      _ <- Clock.currentTime(TimeUnit.MILLISECONDS).debugThread
      _ <- ticks.update(_ + 1)
      _ <- tickingClock
  ...
  //  def run = atomicMOL.debugThread *> mol.debugThread
//  def run = demoConcurrentWorkImpure()
//  def run = demoConcurrentWorkPure()
//  def run = tickingClockImpure()
