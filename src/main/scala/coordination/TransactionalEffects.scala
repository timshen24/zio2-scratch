...
object TransactionalEffects extends ZIOAppDefault {
  ...
  // STM(Software transactional memories) = "atomic effect", which cannot be interrupted
  ...
  // the above transferMoney is buggy!
  def exploitBuggyBank() = for {
    sender <- Ref.make(1000L)
    receiver <- Ref.make(0L)
    fib1 <- transferMoney(sender, receiver, 1000).fork // 因为：这个函数体，它不是atomic的！sender.update(_ - amount)可能多个fiber同时执行这条
    fib2 <- transferMoney(sender, receiver, 1000).fork
    _ <- (fib1 zip fib2).join
//    _ <- fib1.join
//    _ <- fib2.join
    _ <- receiver.get.debugThread // the receiver should NEVER be > 1000
  } yield ()

  def loop(effect: ZIO[Any, String, Unit], i: Int): ZIO[Any, Nothing, Unit] = // bug现原形
    if (i > 10000)
      ZIO.unit
    else
      effect.ignore *> loop(effect, i + 1)

  // STM implementation 修复bug
