 ...
 * Why: 1-allows inter-fiber communication; 2-avoids busy waiting; 3-maintains thread safety
 * Use-cases: 1-producer consumer problems; 2-sending data between fibers 3-notification mechanisms
 */
  ...
  ZIO[R, Nothing, Either[(Exit[E, A], Fiber[E, B]), (Fiber[E, A], Exit[E, B])]] =
    ZIO.uninterruptibleMask { restore =>
      for {
        ...
        fibA <- zioa.onExit(exita => promise.succeed(Left(exita))).fork // finalizer
        fibB <- ziob.onExit(exitb => promise.succeed(Right(exitb))).fork // finalizer
