...
object BlockingEffects extends ZIOAppDefault {
  ...
  val program: ZIO[Any, Nothing, List[Unit]] = ZIO.foreachPar((1 to 100).toList)(blockingTask)
  // thread starvation
  ...
  // can use attemptBlockingInterrupt to do real interrupt
  // based on jvm native Thread.interrupt -> InterruptedException
