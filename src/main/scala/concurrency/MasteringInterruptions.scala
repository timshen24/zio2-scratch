...
object MasteringInterruptions extends ZIOAppDefault {
  ...
  // ZIO.uninterruptible (recommended)
  ...
  val zioComposed2 = (zio1 *> zio2.interruptible *> zio3).uninterruptible // inner scopes override outer scopes, quite rare
...
//    _ <- ZIO.sleep(2.seconds)
  ...
  // uninterruptibleMask is used to define which parts of ZIO is interruptible and which is not
  val authFlow = ZIO.uninterruptibleMask { restore =>
    // EVERYTHING is uninterruptible...except those wrapped with restore(...), which is interruptible
    for {
